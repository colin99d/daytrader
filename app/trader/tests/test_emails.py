...
class EmailTestCase(TransactionTestCase):
    ...
    def test_email_filters_decisions(self):
        ...
        Decision.objects.create(stock=stock1, algorithm=algo1, trade_date=make_aware(datetime(2021, 7, 6, 12, 0)), long=True)
        Decision.objects.create(stock=stock2, algorithm=algo1, trade_date=make_aware(datetime(2021, 7, 7, 12, 0)), long=True)
        Decision.objects.create(stock=stock3, algorithm=algo2, trade_date=make_aware(datetime(2021, 7, 6, 12, 0)), long=True)
        Decision.objects.create(stock=stock4, algorithm=algo2, trade_date=make_aware(datetime(2021, 7, 7, 12, 0)), long=True)
        setattr(self.user,"selected_algo",algo1)
        self.user.save()
        daily_email(self.user)
