...
class CeleryTestCase(TestCase):
    ...
    def test_begin_day(self):
        ...
        user.save()
        Stock.objects.create(ticker = "TSLA", volume=10000,price=0,listed=True)
        Stock.objects.create(ticker = "AAPL", volume=2000,price=1000,listed=True)
        Stock.objects.create(ticker = "AMZN", volume=3000,price=83,listed=True)
        Stock.objects.create(ticker = "GME", volume=4000,price=51,listed=True)
        s5 = Stock.objects.create(ticker = "F", volume=5000,price=26,listed=True)
        Stock.objects.create(ticker = "T", volume=6000,price=132,listed=True)
        s7 = Stock.objects.create(ticker = "CANO", volume=7000,price=56,listed=True)
        Stock.objects.create(ticker = "CMAX", volume=8000,price=200,listed=True)
        Stock.objects.create(ticker = "IWV", volume=20000,price=10,listed=False)
    ...
    def test_begin_day_lots_more(self):
        ...
        setattr(user,"selected_algo",Algorithm.objects.create(name="Z-score daytrader",public=True))
        user.save()
        Stock.objects.create(ticker = "TSLA", volume=1000,price=20,listed=True)
        s2 = Stock.objects.create(ticker = "AAPL", volume=2000,price=1000,listed=True)
        Stock.objects.create(ticker = "AMZN", volume=3000,price=83,listed=True)
        Stock.objects.create(ticker = "GME", volume=4000,price=51,listed=True)
        Stock.objects.create(ticker = "F", volume=5000,price=26,listed=True)
        s6 = Stock.objects.create(ticker = "T", volume=6000,price=132,listed=True)
        Stock.objects.create(ticker = "CANO", volume=7000,price=56,listed=True)
    ...
    def test_update_ticker_information_delists_properly(self):
        ...
        items = json.loads(items.get())
        for item in items:
            self.assertFalse(item["fields"]["active"])
            self.assertFalse(item["fields"]["listed"])
